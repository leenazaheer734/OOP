...
namespace bankGUI.DL
{
    class LoanDL
    {
        ...
        public static void SetrejectLoanReqStatus(Client c)
        {
            for (int i = 0; i < loansList.Count; i++)
            {
                if (checkClientMatch(c, loansList[i]))
                {
                    loansList[i].LoanStatus = 3;
                }
            }
        }
        ...
        public static Loan findLoanNotRejected(Client u)
        {
            foreach (Loan l in LoanDL.getLoanList())
            {
                if (checkClientMatch(u, l)&&  l.LoanStatus != 3)
                {
                    return l;
                }
            }
        ...
        {
            if (l.Loanclient.ClientCred.Username == u.ClientCred.Username && l.Loanclient.Account.AccountNumber == u.Account.AccountNumber)
                return true;
            else
                return false;
        }
        public static void storeLoanInFile()
        {
            ...
            foreach (Loan l in loansList)
            {
                if (l.PledgeItem is Gold)
                    myFile.WriteLine(l.Loanclient.ClientCred.Username + "," + l.Loanclient.Account.AccountNumber + "," + l.ReqDate + "," + l.ApprovalDate + "," + l.ReturningDate + "," + l.ReqAmount + "," + l.GivenAmount + "," + l.Interset + "," + l.LoanStatus + "," + l.Purpose + "," + l.PledgeItem.getValue() + "," + l.PledgeItem.getWeight() + "," + l.PledgeItem.getPurity());
                else if (l.PledgeItem is Property)
                    myFile.WriteLine(l.Loanclient.ClientCred.Username + "," + l.Loanclient.Account.AccountNumber + "," + l.ReqDate + "," + l.ApprovalDate + "," + l.ReturningDate + "," + l.ReqAmount + "," + l.GivenAmount + "," + l.Interset + "," + l.LoanStatus + "," + l.Purpose + "," + l.PledgeItem.getValue() + "," + l.PledgeItem.getPropertyAddress());
            }
            myFile.Flush();
            myFile.Close();
        }
        public static List<Loan> viewRequestededLoans()
        {
        ...
        public static List<Loan> viewApprovedLoans()
        {
            List<Loan> loans = new List<Loan>();
            foreach (Loan l in LoanDL.getLoanList())
            {
